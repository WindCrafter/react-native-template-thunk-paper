...
export const removeVietnameseTonesHelper = (str: string) => {
  if (!str) {
    return ""
  }
...
export const parseJSONHelper = (str?: string) => {
  if (!str) {
    return {}
  }
  try {
...
export function opacityHelperHelper(color: string, opacity: number): string {
  // coerce values so ti is between 0 and 1.
  const _opacity = Math.round(Math.min(Math.max(opacity || 1, 0), 1) * 255);
  return color + _opacity.toString(16).toUpperCase();
}

export function formatBytesHelper(bytes, decimals = 2) {
 ...
 * @param n is number
 */
export function isNonEmptyNumberHelper(n: number | undefined | null) {
 ...
 *  @param s is string
 */
export function isNonEmptyStringHelper(s: string | undefined | null) {
...
export const isNullOrEmptyHelper = (value: any) => {
  return !(typeof (value) !== 'undefined' || value != null);
};

// check null or Undefinded
export const isNullOrUndefinedHelper = (data: any) => {
  return (data == null || data === undefined);
};

export const convertNumberHelper = (n, convertB = true, convertM = true, convertK = true) => {
...
export const isNotNumberStringHelper = (value: string) => {
  return isNaN(parseInt(value.trim())) || value.trim() !== `${parseInt(value.trim())}`
}

export function getValueFromPathHelper(obj, path) {
...
export function setNestedValueHelper(obj, path, value) {
...
export function toHHMMSSHelper(secs: number) {
...
export function truncateStringLogBugsHelper(inputString: string, maxLength: number) {
...
export function replaceLastOccurrencesColorHelper(inputString, targetChar, replacementChar) {
...
export const removeCharPhoneHelper = (inputString: string) => {
...
export const convertDataCadastralHelper = (data: any) => {
